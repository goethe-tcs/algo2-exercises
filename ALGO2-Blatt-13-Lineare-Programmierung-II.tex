...
\begin{exercise}[Primal vs Dual \acs{LP}][\athome]
...
\begin{exercise}[Bipartites Maximum Matching \acs{LP}][\href{https://moodle.studiumdigitale.uni-frankfurt.de/moodle/mod/assign/view.php?id=245954}{Moodle}\athome]\
...
%	\end{enumerate}
	Das \emph{Bipartite Maximum Matching Problem} kann als \acs{LP} formuliert werden. Die Eingabe ist ein bipartiter Graph $G = (U \cup V, E)$, wobei $E \subseteq U \times V$ und die Ausgabe ist ein größtes Matching in $G$.
	\begin{enumerate}
		\item\medium Stelle ein lineares Programm auf, das für jede Kante eine Variable besitzt.
		\item\hard Stelle das duale \acs{LP} aus \emph{Teil a)} auf. Was repräsentieren die dualen Variablen und die Zielfunktion? Welches Problem ist das?
	\end{enumerate}
\end{exercise}

\begin{exercise}[Schulzuweisungs \acs{ILP}][\atschool\medium]
...
\begin{exercise}[\acp{ILP} sind NP-schwer][\href{https://moodle.studiumdigitale.uni-frankfurt.de/moodle/mod/assign/view.php?id=245955}{Moodle}\athome\easy]
	...
	%	\emph{Hint: You want a transformation of a 3-CNF formula with $m$ clauses over $n$ variables to $P$.}
		...
		Zeige, dass das \emph{Integer Feasability Problem} \NP-schwer ist, indem du eine Reduktion ausgehend von \textsc{3-Sat} durchführst.
	
		\emph{Hinweis: Es soll eine \textsc{3CNF} Formel mit $m$ Klauseln und $n$ Variablen zu $P$ transformiert werden.}
	\end{exercise}

\begin{exercise}[Schief-symmetrische Matrizen][\atschool\medium]\
...
%	\end{enumerate}
	Eine Matrix $A = (a_{ij})$ wird genau dann \emph{schief-symmetrisch} genannt, wenn $a_{ji} = -a_{ij}$ für alle Indizes $i, j$ gilt; insbesondere ist jede schief-symmetrische Matrix quadratisch. Ein kanonisches lineares Programm ${\max\{c^T x  \; | \; Ax \leq b, x \geq 0\}}$ ist \emph{selbst-dual}, wenn die Matrix $A$ schief-symmetrisch ist und für den Zielvektor~$c$ und den Vektor~$b$ der Nebenbedingungen folgender Zusammenhang besteht: $c = -b$.
	
	% \begin{enumerate}
	Beweise, dass jedes selbst-duale lineare Programm (P) syntaktisch äquivalent zu seinem dualen linearen Programm (D) ist.
...
\begin{exercise}[Totale Unimodularität]
...
%	\end{enumerate}
	...
	\begin{enumerate}
		\item\medium Sei $A$ eine beliebige total unimodulare Matrix.
		\begin{enumerate}
		...
		\end{enumerate}
		\item\hard Beweise, dass das kanonische lineare Programm $\max\{c^T x  \; | \; Ax \leq b, x \geq 0\}$ für eine beliebige total unimodulare Matrix $A$ und einen beliebigen ganzzahligen Vektor $b$ eine ganzzahlige optimale Lösung hat.
		\tipp{Verwende die Cramersche Regel}
	\end{enumerate}
\end{exercise}
\end{document}
