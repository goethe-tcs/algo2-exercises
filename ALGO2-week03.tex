...
\begin{aufgabe}[Eulerkreise in gemischten Graphen]
    ...
    Im folgenden arbeiten wir mit gemischten Graphen. Das sind Graphen, in denen einige Kanten gerichtet sind und einige ungerichtet. Wir nehmen an, dass die Graphen verbunden sind, beispielsweise sind die Graphen verbunden, wenn die Richtung aller Kanten ignoriert wird.
    Wir wollen einen Algorithmus, der entscheidet, ob es möglich ist, den ungerichteten Kanten eine Richtung zu geben, sodass der Graph einen Eulerkreis hat. Im Beispiel können die ungerichteten Kanten des gemischten Graphen links so gerichtet werden, dass ein Eulerkreis entsteht (die Nummern der Kanten geben die Besuchsreihenfolge an).
    ...
    \begin{figure}[ht]
    	...
    	\begin{minipage}[b]{0.5\textwidth}
			...
			\def\list {v0/v2/2, v1/v3/9, v2/v6/3, v3/v0/7, v3/v2/10, v5/v4/12, v7/v4/5}  % directed edges
			\foreach \u\v\w in \list
			{	\draw[-{Stealth[length=3mm, width=2mm]}] (\u) -- (\v) [midway, left] {};
			}
			
			\def\vertical {v0/v1/8, v2/v5/11, v4/v3/6, v6/v7/4}  % undirected edges
			\foreach \u\v\weight in \vertical
			{	\draw[-{Stealth[length=3mm, width=2mm]}, ultra thick] (\u) -- (\v) [midway, left] {};
			}
			
			\path (v4) edge[-{Stealth[length=3mm, width=2mm]}, ultra thick, bend left=90] node [midway, below] {$1$} (v0);
		\end{tikzpicture}
